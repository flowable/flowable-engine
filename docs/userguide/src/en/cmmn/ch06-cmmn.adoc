...
Historic data related to the case execution is collected for special constructs like Tasks (as seen above), milestones, cases, variables and plan items in general.
This data is persisted while the case definition is deployed, surviving long after its instances have executed.
Access to the historic data is provided as query api's by the _CmmnHistoryService_
