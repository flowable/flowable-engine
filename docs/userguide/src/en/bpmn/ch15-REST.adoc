...
You can activate the alternative behavior with the scope that is applied to variables REST. This new behavior ensures that the scope of the variables is respected. It is activated by setting the property withScopedVariables of JSON to true.

*Complete a task with scoped variables - Body JSON:*

[source,json,linenums]
----
{
  "action": "complete",
  "withScopedVariables": true,
  "variables": []
}
----
