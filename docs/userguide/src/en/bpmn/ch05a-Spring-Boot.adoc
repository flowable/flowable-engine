...
o.a.e.i.a.DefaultAsyncJobExecutor        : Starting up the default async job executor [org.flowable.spring.job.service.SpringAsyncExecutor].
...
It's possible to get a hold of the process engine configuration by implementing the _org.flowable.spring.boot.ProcessEngineConfigurationConfigurer_ interface. This can be useful for advanced configuration settings or simply because a property has not been exposed (yet). For example:
