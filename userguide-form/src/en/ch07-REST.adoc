...
The Flowable REST web application is using Spring Java Configuration for starting the Flowable Form Engine, defining the basic authentication security using Spring security and to define the variable converters for specific variable handling.
...
Parameters that are part of the url (e.g. the deploymentId parameter in ++http://host/flowable-rest/form-api/form-repository/deployments/{deploymentId}++)
...
Parameters added as query-string in the URL (e.g. the name parameter used in ++http://host/flowable-rest/form-api/form-repository/deployments?name=Deployment++) can have the following types and are mentioned in the corresponding REST-API documentation:
...
Paging and order parameters can be added as query-string in the URL (e.g. the name parameter used in ++http://host/flowable-rest/form-api/form-repository/deployments?sort=name++).
...
|resourceId|Yes|String|The id of the resource to get the data for. *Make sure you URL-encode the resourceId in case it contains forward slashes. Eg: use 'forms%2Fmy-form.form' instead of 'forms/my-form.form'.*
...
|deploymentId|No|String|Only return form definitions which are part of a deployment with the given id.
...
|===============
|Parameter|Required|Value|Description
|formDefinitionId|Yes|String|The id of the process definition to get.
...
|===============
|Parameter|Required|Value|Description
|formDefinitionId|Yes|String|The id of the form definition to get the resource data for.
...
|===============
|Parameter|Required|Value|Description
|formDefinitionId|Yes|String|The id of the form definition to get the model for.
...
|===============
|Parameter|Required|Value|Description
|formInstanceId|Yes|String|The id of the form instance to get.
...
|400|Indicates either the form definition was not found (based on id or key), no form instance is store by sending the given message or an invalid variable has been passed. Status description contains additional information about the error.
...
|formDefinitionId|No|String|Only return form instances with the given form definition id.
|formDefinitionIdLike|No|String|Only return form instances with a form definition id like the given value.
|taskId|No|String|Only return form instances with the given task id.
|taskIdLike|No|String|Only return form instances with a task id like the given value.
|processInstanceId|No|String|Only return form instances with the given process instance id.
|processInstanceIdLike|No|String|Only return form instances with a process instance id like the given value.
|processDefinitionId|No|String|Only return form instances with the given process definition id.
|processDefinitionIdLike|No|String|Only return form instances with a process definition id like the given value.
