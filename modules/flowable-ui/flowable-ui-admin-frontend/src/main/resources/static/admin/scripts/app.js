        ...
        .run(['$rootScope', '$http', '$timeout', '$location', '$cookies', '$modal', '$translate', '$window',
            function($rootScope, $http, $timeout, $location, $cookies, $modal, $translate, $window) {
        		...
        		$rootScope.loadServerConfig = function(callbackAfterLoad) {
                    $http({method: 'GET', url: FlowableAdmin.Config.adminContextRoot + 'rest/server-configs'}).
        		...
        		$http.get(FlowableAdmin.Config.adminContextRoot + 'rest/account')
	        	...
	        	$rootScope.loadProcessDefinitionsCache = function() {
                    var promise = $http({
                        method: 'GET',
                        url: FlowableAdmin.Config.adminContextRoot + 'rest/admin/process-definitions?size=100000000'
                    }).success(function (data, status, headers, config) {
                        return data;
                    }).error(function (data, status, headers, config) {
                        return {'status': false};
	            ...
	            $rootScope.loadCaseDefinitionsCache = function() {
                    var promise = $http({
                        method: 'GET',
                        url: FlowableAdmin.Config.adminContextRoot + 'rest/admin/case-definitions?size=100000000'
                    }).success(function (data, status, headers, config) {
                        return data;
                    }).error(function (data, status, headers, config) {
                        return {'status': false};
