...
public class ChangeTenantIdDmnCompleteCmd implements Command<ChangeTenantIdResult> {
    ...
    public ChangeTenantIdResult execute(CommandContext commandContext) {
        ...
        long changeTenantIdHistoricDecisionExecutions = dmnEngineConfiguration
                .getHistoricDecisionExecutionEntityManager().changeTenantIdHistoricDecisionExecutions(sourceTenantId,
                        targetTenantId, onlyInstancesFromDefaultTenantDefinitions);
        return new DefaultChangeTenantIdResult(
                Collections.singletonMap(HISTORIC_DECISION_EXECUTIONS, changeTenantIdHistoricDecisionExecutions));
    }
