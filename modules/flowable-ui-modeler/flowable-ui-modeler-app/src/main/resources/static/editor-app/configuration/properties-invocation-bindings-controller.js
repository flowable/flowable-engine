...
angular.module('flowableModeler').controller('FlowableInvocationBindingCtrl',
    ['$scope', '$modal', '$timeout', '$translate', function ($scope, $modal, $timeout, $translate) {

        // Config for the modal window
        var opts = {
            template: 'editor-app/configuration/properties/invocation-bindings-popup.html?version=' + Date.now(),
            scope: $scope
        };
...
angular.module('flowableModeler').controller('FlowableInvocationBindingPopupCtrl',
    ['$scope', '$q', '$translate', '$timeout', function ($scope, $q, $translate, $timeout) {

        // Put json representing form properties on scope
        if ($scope.property.value !== undefined && $scope.property.value !== null
            && $scope.property.value.invocationBindings !== undefined
            && $scope.property.value.invocationBindings !== null) {
            // Note that we clone the json object rather then setting it directly,
            // this to cope with the fact that the user can click the cancel button and no changes should have happended
            $scope.invocationBindings = angular.copy($scope.property.value.invocationBindings);
            
            for (var i = 0; i < $scope.invocationBindings.length; i++) {
			    var formProperty = $scope.invocationBindings[i];
        ...
        var namePromise = $translate('PROPERTY.INVOCATIONBINDING.PARAMETER');
        var typePromise = $translate('PROPERTY.INVOCATIONBINDING.EXPRESSION');

        $q.all([namePromise, typePromise]).then(function (results) {
            $scope.labels.parameterLabel = results[0];
            $scope.labels.expressionLabel = results[1];
            $scope.translationsRetrieved = true;

            // Config for grid
            $scope.gridOptions = {
                data: $scope.invocationBindings,
                headerRowHeight: 28,
                ...
                enableSorting: false,
                columnDefs: [
                    {field: 'parameter', displayName: $scope.labels.parameterLabel},
                    {field: 'expression', displayName: $scope.labels.expressionLabel}]
            };
            ...
            $scope.enumGridOptions = {
                ...
                enableColumnMenus: false,
                enableSorting: false,
                columnDefs: [{ field: 'parameter', displayName: $scope.labels.parameterLabel },
                { field: 'expression', displayName: $scope.labels.expressionLabel}]
            }
        ...
        $scope.addNewProperty = function () {
            ...
            $scope.invocationBindings.push(newProperty);
        ...
        $scope.removeProperty = function () {
            ...
            if (selectedItems && selectedItems.length > 0) {
                var index = $scope.invocationBindings.indexOf(selectedItems[0]);
                $scope.gridApi.selection.toggleRowSelection(selectedItems[0]);
                $scope.invocationBindings.splice(index, 1);

                if ($scope.invocationBindings.length == 0) {
                    $scope.selectedProperty = undefined;
                }

                $timeout(function() {
                    if ($scope.invocationBindings.length > 0) {
                        $scope.gridApi.selection.toggleRowSelection($scope.invocationBindings[0]);
                    }
                });
            }
        };
        ...
        $scope.movePropertyUp = function () {
            ...
            if (selectedItems && selectedItems.length > 0) {
                ...
                if (index != 0) { // If it's the first, no moving up of course
                    var temp = $scope.invocationBindings[index];
                    $scope.invocationBindings.splice(index, 1);
                    $timeout(function(){
                        $scope.invocationBindings.splice(index + -1, 0, temp);
                        $timeout(function() {
        ...
        $scope.movePropertyDown = function () {
            ...
            if (selectedItems && selectedItems.length > 0) {
                var index = $scope.invocationBindings.indexOf(selectedItems[0]);
                if (index != $scope.invocationBindings.length - 1) { // If it's the last element, no moving down of course
                    var temp = $scope.invocationBindings[index];
                    $scope.invocationBindings.splice(index, 1);
                    $timeout(function(){
                        $scope.invocationBindings.splice(index + 1, 0, temp);
        ...
        $scope.save = function () {
            ...
            if ($scope.invocationBindings.length > 0) {
                ...
                $scope.property.value.invocationBindings = $scope.invocationBindings;
            } else {
