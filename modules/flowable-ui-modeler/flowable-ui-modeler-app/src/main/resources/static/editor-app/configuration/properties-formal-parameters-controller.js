...
angular.module('flowableModeler').controller('FlowableFormalParametersCtrl',
    ['$scope', '$modal', '$timeout', '$translate', function ($scope, $modal, $timeout, $translate) {

        // Config for the modal window
        var opts = {
            template: 'editor-app/configuration/properties/formal-parameters-popup.html?version=' + Date.now(),
            scope: $scope
        };
...
angular.module('flowableModeler').controller('FlowableFormalParametersPopupCtrl',
    ['$scope', '$q', '$translate', '$timeout', function ($scope, $q, $translate, $timeout) {

        // Put json representing form properties on scope
        if ($scope.property.value !== undefined && $scope.property.value !== null
            && $scope.property.value.formalParameters !== undefined
            && $scope.property.value.formalParameters !== null) {
            // Note that we clone the json object rather then setting it directly,
            // this to cope with the fact that the user can click the cancel button and no changes should have happended
            $scope.formalParameters = angular.copy($scope.property.value.formalParameters);
            
            for (var i = 0; i < $scope.formalParameters.length; i++) {
			    var formProperty = $scope.formalParameters[i];
        ...
        var namePromise = $translate('PROPERTY.FORMALPARAMETERS.NAME');
        var typePromise = $translate('PROPERTY.FORMALPARAMETERS.TYPE');
        ...
        $scope.addNewProperty = function () {
            ...
            $scope.formalParameters.push(newProperty);
        ...
        $scope.removeProperty = function () {
            ...
            if (selectedItems && selectedItems.length > 0) {
                var index = $scope.formalParameters.indexOf(selectedItems[0]);
                $scope.gridApi.selection.toggleRowSelection(selectedItems[0]);
                $scope.formalParameters.splice(index, 1);

                if ($scope.formalParameters.length == 0) {
                    $scope.selectedProperty = undefined;
                }

                $timeout(function() {
                    if ($scope.formalParameters.length > 0) {
                        $scope.gridApi.selection.toggleRowSelection($scope.formalParameters[0]);
                    }
                });
            }
        };
        ...
        $scope.movePropertyUp = function () {
            ...
            if (selectedItems && selectedItems.length > 0) {
                ...
                if (index != 0) { // If it's the first, no moving up of course
                    var temp = $scope.formalParameters[index];
                    $scope.formalParameters.splice(index, 1);
                    $timeout(function(){
                        $scope.formalParameters.splice(index + -1, 0, temp);
                        $timeout(function() {
        ...
        $scope.movePropertyDown = function () {
            ...
            if (selectedItems && selectedItems.length > 0) {
                var index = $scope.formalParameters.indexOf(selectedItems[0]);
                if (index != $scope.formalParameters.length - 1) { // If it's the last element, no moving down of course
                    var temp = $scope.formalParameters[index];
                    $scope.formalParameters.splice(index, 1);
                    $timeout(function(){
                        $scope.formalParameters.splice(index + 1, 0, temp);
        ...
        $scope.save = function () {
            ...
            if ($scope.formalParameters.length > 0) {
                ...
                $scope.property.value.formalParameters = $scope.formalParameters;
            } else {
