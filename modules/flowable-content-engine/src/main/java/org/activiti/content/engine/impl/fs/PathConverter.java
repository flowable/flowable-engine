...
/**
 * Converts between a unique content index and a relative {@link File} path.
 * Uses a nested tree of folders (depth based on iterationDepth) with a maximum
 * number of children (based on blockSize). The leaves of the trees will be the
 * actual content files. This way, no folder will have more children than the
 * 'blockSize' set, keeping the folders balanced.
 * 
...
public class PathConverter {
  ...
  /**
   * @return a path representing the content with the given unique content
   *         index.
   */
  public File getPathForId(BigInteger id) {
    ...
    if (!id.equals(BigInteger.ZERO)) {
      throw new ContentStorageException("ID out of range of content storage limit: " + blockSize.pow(iterationDepth).subtract(BigInteger.ONE));
    }
