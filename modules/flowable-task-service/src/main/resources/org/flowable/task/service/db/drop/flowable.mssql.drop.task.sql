...
if exists (SELECT name FROM sysindexes WHERE name = 'FLW_IDX_TASK_LOG_NUMBER') drop index FLW_TSK_LOG.FLW_IDX_TASK_LOG_NUMBER;

if exists (select TABLE_NAME from INFORMATION_SCHEMA.TABLES where TABLE_NAME = 'ACT_RU_TASK') drop table ACT_RU_TASK;
if exists (select TABLE_NAME from INFORMATION_SCHEMA.TABLES where TABLE_NAME = 'FLW_TSK_LOG') drop table FLW_TSK_LOG;