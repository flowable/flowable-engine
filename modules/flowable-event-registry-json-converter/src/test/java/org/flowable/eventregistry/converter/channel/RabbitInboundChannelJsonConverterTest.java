 ...
 */
class RabbitInboundChannelJsonConverterTest extends AbstractChannelConverterTest {

    @Test
    void convertJsonToModel() {
        ChannelModel channelModel = readJson("org/flowable/eventregistry/converter/channel/simpleRabbitInboundChannel.json");
    ...
    protected void validateModel(ChannelModel channelModel) {
        assertThat(channelModel)
            .isInstanceOfSatisfying(RabbitInboundChannelModel.class, model -> {
                ...
                assertThat(model.isExclusive()).isTrue();
                assertThat(model.getPriority()).isEqualTo("10");
                assertThat(model.getAdmin()).isEqualTo("rabbitAdmin");
                assertThat(model.getConcurrency()).isEqualTo("3");
                assertThat(model.getExecutor()).isEqualTo("rabbitTaskExecutor");
                assertThat(model.getAckMode()).isEqualTo("NONE");
            });
    }
