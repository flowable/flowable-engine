 ...
 * @author Joram Barrez
 */
public class IdmEngineDropScriptsTest extends PluggableFlowableIdmTestCase {
    ...
    public void testDropSchema() {
        ...
        idmEngineConfiguration.getCommandExecutor().execute(new SchemaOperationsEngineDropDbCmd(idmEngineConfiguration.getEngineScopeType()));
        assertThat(countTables()).isZero();

        idmEngineConfiguration.getCommandExecutor().execute(new SchemaOperationsEngineBuild(idmEngineConfiguration.getEngineScopeType()));
        assertThat(countTables()).isEqualTo(originalTableCount);
    }

    private long countTables() {
        return idmEngineConfiguration.getIdmManagementService().getTableCount().
                keySet().stream().filter(tableName -> tableName.toLowerCase().startsWith("act_id")).count();
    }
