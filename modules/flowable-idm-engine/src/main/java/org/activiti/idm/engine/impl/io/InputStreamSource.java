 ...
 */
package org.activiti.idm.engine.impl.io;
...
public class InputStreamSource implements StreamSource {
  ...
  public InputStream getInputStream() {
    if (bytes == null) {
      try {
        ...
        throw new ActivitiIdmException("Could not read from inputstream", e);
      }
    }
  ...
  public  byte[] getBytesFromInputStream(InputStream inStream) throws IOException {
    ...
    if (offset < bytes.length) {
      throw new ActivitiIdmException("Could not completely read inputstream ");
    }
