  ...
  protected void addOrder(String column, String sortOrder, NullHandlingOnOrder nullHandlingOnOrder) {
    ...
    if (nullHandlingOnOrder != null) {
      ...
      if (nullHandlingOnOrder.equals(NullHandlingOnOrder.NULLS_FIRST)) {
        ...
        if (IdmEngineConfiguration.DATABASE_TYPE_H2.equals(databaseType) || IdmEngineConfiguration.DATABASE_TYPE_HSQL.equals(databaseType)
            || IdmEngineConfiguration.DATABASE_TYPE_POSTGRES.equals(databaseType) || IdmEngineConfiguration.DATABASE_TYPE_ORACLE.equals(databaseType)) {
          orderBy = orderBy + defaultOrderByClause + " NULLS FIRST";
        } else if (IdmEngineConfiguration.DATABASE_TYPE_MYSQL.equals(databaseType)) {
          orderBy = orderBy + "isnull(" + column + ") desc," + defaultOrderByClause;
        } else if (IdmEngineConfiguration.DATABASE_TYPE_DB2.equals(databaseType) || IdmEngineConfiguration.DATABASE_TYPE_MSSQL.equals(databaseType)) {
