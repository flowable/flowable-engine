...
public enum ActivitiIdmEventType implements ActivitiEventType {
  ...
  public static final ActivitiIdmEventType[] EMPTY_ARRAY = new ActivitiIdmEventType[] {};

  /**
   * @param string
   *          the string containing a comma-separated list of event-type names
   * @return a list of {@link ActivitiIdmEventType} based on the given list.
   * @throws IllegalArgumentException
   *           when one of the given string is not a valid type name
   */
  public static ActivitiIdmEventType[] getTypesFromString(String string) {
    List<ActivitiIdmEventType> result = new ArrayList<ActivitiIdmEventType>();
    if (string != null && !string.isEmpty()) {
      String[] split = string.split(",");
      for (String typeName : split) {
        boolean found = false;
        for (ActivitiIdmEventType type : values()) {
