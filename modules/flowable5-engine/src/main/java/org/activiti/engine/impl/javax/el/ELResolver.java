...
public abstract class ELResolver {
     ...
     * designtime. For instance, it may be unsafe to attempt a resolution at design time if the ELResolver needs access to a resource that is only available at runtime and no acceptable simulated
     ...
     * {@link #setValue(ELContext, Object, Object, Object)} method. If this resolver handles the given (base, property) pair, the propertyResolved property of the ELContext object must be set to true
     * by the resolver, before returning. If this property is not true after this method is called, the caller should ignore the return value. This is not always the same as getValue().getClass(). For
    ...
    /**
     * Attempts to resolve the given property object on the given base object. If this resolver handles the given (base, property) pair, the propertyResolved property of the ELContext object must be
     * set to true by the resolver, before returning. If this property is not true after this method is called, the caller should ignore the return value.
     * 
     * @param context
     *            The context of this evaluation.
     * @param base
