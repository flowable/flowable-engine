...
public class MultiInstanceUserTaskEventsTest extends PluggableFlowableTestCase {
    ...
    protected void tearDown() throws Exception {
        ...
        if (testListener != null) {
            ...
            processEngineConfiguration.getEventDispatcher()
                .removeEventListener(testListener);
        }
    ...
    public void testMultiInstanceCancelledWhenFlowToTerminateEnd() throws Exception {
        ...
        Execution task1Execution = runtimeService.createExecutionQuery()
            .activityId("task1")
            .singleResult();
        Execution boundaryExecution = runtimeService.createExecutionQuery()
            .activityId("cancelBoundaryEvent1")
            .singleResult();
        List<Execution> multiExecutions = runtimeService.createExecutionQuery()
            .activityId("task2")
            .list();
        assertEquals(2, multiExecutions.size());
        String multiExecutionId1 = multiExecutions.get(0)
            .getId();
        String multiExecutionId2 = multiExecutions.get(1)
            .getId();
        ...
        int idx = 0;
        FlowableEvent flowableEvent = testListener.getEventsReceived()
            .get(idx++);
        ...
        FlowableActivityEvent activityEvent = (FlowableActivityEvent) testListener.getEventsReceived()
            .get(idx++);
        ...
        activityEvent = (FlowableActivityEvent) testListener.getEventsReceived()
            .get(idx++);
        ...
        FlowableEntityEvent entityEvent = (FlowableEntityEvent) testListener.getEventsReceived()
            .get(idx++);
        ...
        entityEvent = (FlowableEntityEvent) testListener.getEventsReceived()
            .get(idx++);
        assertEquals(FlowableEngineEventType.TASK_CREATED, entityEvent.getType());
        ...
        assertEquals(18, idx);
        assertEquals(18, testListener.getEventsReceived()
            .size());
    }
    ...
    @Deployment(resources = {
        "org/flowable/engine/test/api/event/MultiInstanceUserTaskEventsTest.testMultiInstanceCompleteCondition.bpmn20.xml" })
    public void testMultiInstanceCompleteCondition() throws Exception {
        ...
        JobTestHelper.waitForJobExecutorToProcessAllJobs(processEngineConfiguration, managementService, 100000L, 500L);
        activityEvent = (FlowableActivityEvent) testListener.getEventsReceived()
            .get(idx++);
    ...
    public void testMultiInstanceComplete() throws Exception {
        ...
        multiExecutions = runtimeService.createExecutionQuery()
            .activityId("task")
            .list();
        assertEquals(2, multiExecutions.size());
    ...
    /**
     * Multi-instance user task cancelled by message boundary event defined on multi-instance user task.
     */
    @Deployment(resources = { "org/flowable/engine/test/api/event/MultiInstanceUserTaskEventsTest.bpmn20.xml" })
    public void testMultiInstanceCancelledByMessageBoundaryEvent() throws Exception {
        ...
        Execution cancelMessageExecution = runtimeService.createExecutionQuery()
            .messageEventSubscriptionName("cancel")
            .singleResult();
        ...
        assertEquals(17, idx);
        assertEquals(17, testListener.getEventsReceived()
            .size());
    }

    /**
     * Multi-instance user task defined in external subprocess. The multi-instance user tasks are cancelled by message boundary event
     * defined on multi-instance user task.
     */
    @Deployment(resources = { "org/flowable/engine/test/api/event/MultiInstanceUserTaskEventsTest.testCallActivityTerminateEnd.bpmn20.xml",
    ...
    public void testMultiInstanceInCallActivityCancelledByMessageBoundaryEvent() throws Exception {
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("multiInstanceCallActivityTerminateEnd");
        assertNotNull(processInstance);
        ...
        // external subprocess
        flowableEvent = testListener.getEventsReceived()
            .get(idx++);
        assertEquals(FlowableEngineEventType.PROCESS_STARTED, flowableEvent.getType());
        ...
        assertEquals(testListener.getEventsReceived()
            .size(), idx);
        testListener.clearEventsReceived();
        ...
        assertEquals(10, idx);
        assertEquals(10, testListener.getEventsReceived()
            .size());
    }

    /**
     * Multi-instance user task defined in external subprocess. The external subprocess and the multi-instance user tasks are cancelled when
     * parent flows to terminate end event.
     */
    @Deployment(resources = { "org/flowable/engine/test/api/event/MultiInstanceUserTaskEventsTest.testCallActivityTerminateEnd.bpmn20.xml",
        "org/flowable/engine/test/api/event/MultiInstanceUserTaskEventsTest.testCalledActivityTerminateEnd.bpmn20.xml" })
    public void testMultiInstanceInCallActivityCancelledWhenFlowToTerminateEnd() throws Exception {
        ...
        testListener.getEventsReceived()
            .clear();
        idx = 0;
        ...
        FlowableCancelledEvent processCancelledEvent = (FlowableCancelledEvent) testListener.getEventsReceived()
            .get(idx++);
        ...
        assertEquals(10, idx);
        assertEquals(idx, testListener.getEventsReceived()
            .size());
    }

    @Deployment(resources = { "org/flowable/engine/test/api/event/MultiInstanceUserTaskEventsTest.testEmbeddedSubprocess.bpmn20.xml" })
    public void testMultiInstanceInSubprocessCancelledWhenFlowToTerminateEnd() throws Exception {
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("multiInstanceEmbeddedSubprocess");
        assertNotNull(processInstance);
    ...
    public void testMultiInstanceSequentialUserTaskEventsWithNormalCompletion() {
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("ID_b8a6875f-6c4b-4864-b369-af6e69c612a5");
        assertNotNull(processInstance);
        String processId = processInstance.getProcessInstanceId();
        int idx = 0;

        FlowableProcessStartedEvent startEvent = (FlowableProcessStartedEvent) testListener.getEventsReceived()
            .get(idx++);
    ...
    class MultiInstanceUserActivityEventListener extends AbstractFlowableEngineEventListener {
        ...
        public MultiInstanceUserActivityEventListener() {
            super(new HashSet<>(Arrays.asList(FlowableEngineEventType.ACTIVITY_STARTED, FlowableEngineEventType.ACTIVITY_COMPLETED,
                FlowableEngineEventType.ACTIVITY_CANCELLED, FlowableEngineEventType.MULTI_INSTANCE_ACTIVITY_STARTED,
                ...
                FlowableEngineEventType.MULTI_INSTANCE_ACTIVITY_CANCELLED, FlowableEngineEventType.TASK_CREATED,
                FlowableEngineEventType.TASK_COMPLETED, FlowableEngineEventType.PROCESS_STARTED, FlowableEngineEventType.PROCESS_COMPLETED,
                FlowableEngineEventType.PROCESS_CANCELLED, FlowableEngineEventType.PROCESS_COMPLETED_WITH_TERMINATE_END_EVENT)));
