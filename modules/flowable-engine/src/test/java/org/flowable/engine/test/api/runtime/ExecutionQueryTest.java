...
public class ExecutionQueryTest extends PluggableFlowableTestCase {
    ...
    @Deployment(resources = {"org/flowable/engine/test/api/runtime/multipleSubProcess.bpmn20.xml",
            "org/flowable/engine/test/api/runtime/subProcess.bpmn20.xml"})
    public void testSubProcessExecutionsByRootId() throws Exception {
        ProcessInstance rootProcessInstance = runtimeService.startProcessInstanceByKey("multipleSubProcessTest");

        List<Execution> processInstances = runtimeService.createExecutionQuery().rootProcessInstanceId(rootProcessInstance.getId()).onlyProcessInstanceExecutions().list();
