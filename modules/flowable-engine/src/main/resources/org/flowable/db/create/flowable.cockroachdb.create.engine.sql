CREATE TABLE ${databaseSchema}ACT_RE_DEPLOYMENT (
    ID_ varchar(64),
    NAME_ varchar(255),
...
CREATE TABLE ${databaseSchema}ACT_RE_MODEL (
...
CREATE TABLE ${databaseSchema}ACT_RU_EXECUTION (
    ID_ varchar(64),
    REV_ integer,
...
CREATE TABLE ${databaseSchema}ACT_RE_PROCDEF (
    ID_ varchar(64) NOT NULL,
    REV_ integer,
    CATEGORY_ varchar(255),
    NAME_ varchar(255),
...
CREATE TABLE ${databaseSchema}ACT_RU_EVENT_SUBSCR (
    ID_ varchar(64) not null,
    REV_ integer,
...
CREATE TABLE ${databaseSchema}ACT_PROCDEF_INFO (
	ID_ varchar(64) not null,
...
CREATE TABLE ${databaseSchema}ACT_RU_ACTINST (
    ID_ varchar(64) not null,
...
create index ACT_IDX_EXEC_BUSKEY on ${databaseSchema}ACT_RU_EXECUTION(BUSINESS_KEY_);
create index ACT_IDX_EXE_ROOT on ${databaseSchema}ACT_RU_EXECUTION(ROOT_PROC_INST_ID_);
create index ACT_IDX_EVENT_SUBSCR_CONFIG_ on ${databaseSchema}ACT_RU_EVENT_SUBSCR(CONFIGURATION_);
create index ACT_IDX_VARIABLE_TASK_ID on ${databaseSchema}ACT_RU_VARIABLE(TASK_ID_);

create index ACT_IDX_BYTEAR_DEPL on ${databaseSchema}ACT_GE_BYTEARRAY(DEPLOYMENT_ID_);

create index ACT_IDX_RU_ACTI_START on ${databaseSchema}ACT_RU_ACTINST(START_TIME_);
create index ACT_IDX_RU_ACTI_END on ${databaseSchema}ACT_RU_ACTINST(END_TIME_);
create index ACT_IDX_RU_ACTI_PROC on ${databaseSchema}ACT_RU_ACTINST(PROC_INST_ID_);
create index ACT_IDX_RU_ACTI_PROC_ACT on ${databaseSchema}ACT_RU_ACTINST(PROC_INST_ID_, ACT_ID_);
