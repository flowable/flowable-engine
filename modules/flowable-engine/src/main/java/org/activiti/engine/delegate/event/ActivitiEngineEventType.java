 ...
 * 
 */
public enum ActivitiEngineEventType implements ActivitiEventType {
  ...
  public static final ActivitiEngineEventType[] EMPTY_ARRAY = new ActivitiEngineEventType[] {};

  /**
   * @param string
   *          the string containing a comma-separated list of event-type names
   * @return a list of {@link ActivitiEngineEventType} based on the given list.
   * @throws ActivitiIllegalArgumentException
   *           when one of the given string is not a valid type name
   */
  public static ActivitiEngineEventType[] getTypesFromString(String string) {
    List<ActivitiEngineEventType> result = new ArrayList<ActivitiEngineEventType>();
    if (string != null && !string.isEmpty()) {
      String[] split = StringUtils.split(string, ",");
      for (String typeName : split) {
        boolean found = false;
        for (ActivitiEngineEventType type : values()) {
