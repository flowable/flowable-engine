 ...
 */
public abstract class AbstractServiceTaskParseHandler<T extends ServiceTask> extends AbstractActivityBpmnParseHandler<T> {
    ...
    protected void executeParse(BpmnParse bpmnParse, T serviceTask) {
        ...
        if (StringUtils.isNotEmpty(serviceTask.getType())) {
            ...
            // Webservice
        } else if (ImplementationType.IMPLEMENTATION_TYPE_WEBSERVICE.equalsIgnoreCase(serviceTask.getImplementationType()) && StringUtils.isNotEmpty(
                serviceTask.getOperationRef())) {

            WebServiceActivityBehavior webServiceActivityBehavior = bpmnParse.getActivityBehaviorFactory()
                    .createWebServiceActivityBehavior(serviceTask, bpmnParse.getBpmnModel());
            serviceTask.setBehavior(webServiceActivityBehavior);

        } else {
            LOGGER.warn("One of the attributes 'class', 'delegateExpression', 'type', 'operation', or 'expression' is mandatory on service task {}",
                    serviceTask.getId());
        }
