...
public class AddTransactionEventListenerCommand implements Command<Void> {
    ...
    public Void execute(CommandContext commandContext) {
        ...
        if (types != null) {
            CommandContextUtil.getProcessEngineConfiguration(commandContext).getTransactionDependentEventDispatcher().addEventListener(listener, types);
        } else {
            CommandContextUtil.getProcessEngineConfiguration(commandContext).getTransactionDependentEventDispatcher().addEventListener(listener);
        }
