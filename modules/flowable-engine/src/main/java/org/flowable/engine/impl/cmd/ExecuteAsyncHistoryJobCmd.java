 ...
 * @author Joram Barrez
 */
public class ExecuteAsyncHistoryJobCmd implements Command<Object>, Serializable {

    private static final long serialVersionUID = 1L;

    private static Logger log = LoggerFactory.getLogger(ExecuteAsyncHistoryJobCmd.class);

    protected String jobId;

    public ExecuteAsyncHistoryJobCmd(String jobId) {
        this.jobId = jobId;
    }
    ...
    public Object execute(CommandContext commandContext) {
        ...
        if (job == null) {
            log.debug("History job does not exist anymore and will not be executed. It has most likely been deleted "
                    + "as part of another concurrent part of the process instance.");
            return null;
        }

        if (log.isDebugEnabled()) {
            log.debug("Executing async history job {}", job.getId());
        }

        commandContext.setHideAsyncHistoryExceptions(true);
