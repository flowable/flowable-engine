...
public class EventRegistryEnginePostEngineBuildConsumer implements Consumer<EventRegistryEngine> {
    ...
    public void accept(EventRegistryEngine eventRegistryEngine) {
        EventRegistryEngineConfiguration engineConfiguration = eventRegistryEngine.getEventRegistryEngineConfiguration();
        if (engineConfiguration.getEngineLifecycleListeners() != null) {
            for (EngineLifecycleListener engineLifecycleListener : engineConfiguration.getEngineLifecycleListeners()) {
                engineLifecycleListener.onEngineBuilt(eventRegistryEngine);
            }
        }

        if (engineConfiguration.isEnableEventRegistryChangeDetectionAfterEngineCreate()) {

            eventRegistryEngine.handleDeployedChannelDefinitions();

            if (engineConfiguration.isEnableEventRegistryChangeDetection()) {
                engineConfiguration.getEventRegistryChangeDetectionExecutor().initialize();
            }
        }
    }
}
