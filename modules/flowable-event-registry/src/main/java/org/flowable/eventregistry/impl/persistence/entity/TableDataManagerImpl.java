...
import org.flowable.eventregistry.impl.persistence.AbstractManager;
...
public class TableDataManagerImpl extends AbstractManager implements TableDataManager {
    ...
    public TableDataManagerImpl(EventRegistryEngineConfiguration eventEngineConfiguration) {
        super(eventEngineConfiguration);
    }
    ...
    public List<String> getTablesPresentInDatabase() {
        ...
        try {
            ...
            String catalog = null;
            if (getEventEngineConfiguration().getDatabaseCatalog() != null && getEventEngineConfiguration().getDatabaseCatalog().length() > 0) {
                catalog = getEventEngineConfiguration().getDatabaseCatalog();
            }

            String schema = null;
            if (getEventEngineConfiguration().getDatabaseSchema() != null && getEventEngineConfiguration().getDatabaseSchema().length() > 0) {
                if ("oracle".equals(getDbSqlSession().getDbSqlSessionFactory().getDatabaseType())) {
                    schema = getEventEngineConfiguration().getDatabaseSchema().toUpperCase();
                } else {
                    schema = getEventEngineConfiguration().getDatabaseSchema();
                }
            }
    ...
    protected long getTableCount(String tableName) {
        ...
        Long count = (Long) getDbSqlSession().selectOne("org.flowable.eventregistry.impl.TablePageMap.selectTableCount", Collections.singletonMap("tableName", tableName));
        return count;
    }
