...
public class FlowableProcessAutoDeployBeanFactoryInitializationAotProcessor implements BeanFactoryInitializationAotProcessor {
    ...
    public BeanFactoryInitializationAotContribution processAheadOfTime(ConfigurableListableBeanFactory beanFactory) {
        if (!ClassUtils.isPresent("org.flowable.spring.SpringProcessEngineConfiguration", beanFactory.getBeanClassLoader())) {
            return null;
        }

        if (!beanFactory.containsBean("springProcessEngineConfiguration")) {
            return null;
        }

        FlowableProperties properties = beanFactory.getBeanProvider(FlowableProperties.class)
                .getIfAvailable();
        if (properties == null || !properties.isCheckProcessDefinitions()) {
            return null;
        }
        List<String> locationSuffixes = properties.getProcessDefinitionLocationSuffixes();
        if (locationSuffixes.isEmpty()) {
            return null;
        }
        String locationPrefix = properties.getProcessDefinitionLocationPrefix();
        if (locationPrefix.startsWith(ResourceUtils.CLASSPATH_URL_PREFIX) || locationPrefix.startsWith(ResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX)) {
            return new ProcessAutoDeployResourceContribution(locationPrefix, locationSuffixes, beanFactory);
        }
