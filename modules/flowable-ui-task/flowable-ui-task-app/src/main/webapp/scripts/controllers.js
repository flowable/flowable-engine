...
activitiApp.controller('AddAppDefinitionPopupCrtl', ['$rootScope', '$scope', '$http', '$translate', function ($rootScope, $scope, $http, $translate) {
    ...
    $http({method: 'GET', url: FLOWABLE.CONFIG.contextRoot + '/app/rest/editor/app-definitions'}).
    ...
    $scope.deploy = function() {
        ...
        $http({method: 'POST', url: FLOWABLE.CONFIG.contextRoot + '/app/rest/runtime/app-definitions', data: data}).
            success(function(response, status, headers, config) {
                $scope.loadApps();
