...
/**
 * In-memory {@link DeadLetterJobDataManager} implementation.
 *
 * @author ikaakkola (Qvantel Finland Oy)
 */
public class MemoryDeadLetterJobDataManager extends AbstractJobMemoryDataManager<DeadLetterJobEntity> implements DeadLetterJobDataManager {
    ...
    private boolean filterJob(DeadLetterJobEntity item, DeadLetterJobQueryImpl query) {
        ...
        if (query.isOnlyExternalWorkers()) {
            retVal = QueryUtil.matchReturn(item.getJobType() != null && item.getJobType().equals("externalWorker"), false);
            if (retVal != null) {
                return retVal;
            }
        }
    ...
    private List<Job> sortAndLimit(List<Job> collect, DeadLetterJobQueryImpl query) {
