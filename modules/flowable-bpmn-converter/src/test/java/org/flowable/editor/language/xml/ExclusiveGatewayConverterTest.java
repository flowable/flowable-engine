...
class ExclusiveGatewayConverterTest {
    ...
    @BpmnXmlConverterTest("exclusivegatewaymodel.bpmn")
    void validateModel(BpmnModel model) {
        FlowElement flowElement = model.getMainProcess().getFlowElement("exclusiveGateway");
        assertThat(flowElement).isInstanceOf(ExclusiveGateway.class);

        ExclusiveGateway gateway = (ExclusiveGateway) flowElement;
        assertThat(gateway.isMarkerVisible()).isFalse();
    }
}