 ...
 * 
 */
public enum FlowableVariableServiceEventType implements FlowableEventType {
    ...
    public static final FlowableVariableServiceEventType[] EMPTY_ARRAY = new FlowableVariableServiceEventType[] {};

    /**
     * @param string
     *            the string containing a comma-separated list of event-type names
     * @return a list of {@link FlowableVariableServiceEventType} based on the given list.
     * @throws FlowableIllegalArgumentException
     *             when one of the given string is not a valid type name
     */
    public static FlowableVariableServiceEventType[] getTypesFromString(String string) {
        List<FlowableVariableServiceEventType> result = new ArrayList<>();
        if (string != null && !string.isEmpty()) {
            String[] split = StringUtils.split(string, ",");
            for (String typeName : split) {
                boolean found = false;
                for (FlowableVariableServiceEventType type : values()) {
