 ...
 */
class RequiredRuleCmmnXmlConverterTest {

    @CmmnXmlConverterTest("org/flowable/test/cmmn/converter/requiredRuleWithExtensionElements.cmmn")
    void customExtensionElements(CmmnModel model) {
        Stage mainPlanModel = model.getPrimaryCase()
                .getPlanModel();

        PlanItem planItem = mainPlanModel.getPlanItem("requiredRuleWithExtensionElements");
        PlanItemControl itemControl = planItem.getItemControl();
        assertThat(itemControl).isNotNull();
        RequiredRule rule = itemControl.getRequiredRule();
