...
public class TaskQueryTest extends PluggableFlowableTestCase {
    ...
    public void testQueryByCandidateOrAssignedOr() {
        ...
        org.flowable.task.service.Task assigneeToKermit = taskService.createTaskQuery()
                .or()
                .taskId("invalid")
    ...
    public void testProcessCategoryNotInOr() throws Exception {
        ...
        final org.flowable.task.service.Task task = taskService.createTaskQuery()
                .or()
                .taskId("invalid")
    ...
    public void testProcessInstanceIdInOr() throws Exception {
        ...
        final org.flowable.task.service.Task task = taskService.createTaskQuery()
                .or()
                .taskId("invalid")
                .processInstanceIdIn(Collections.singletonList(processInstance.getId())).singleResult();
        assertNotNull(task);
        assertEquals("theTask", task.getTaskDefinitionKey());
        assertEquals(processInstance.getId(), task.getProcessInstanceId());
        ...
        assertEquals(0, taskService.createTaskQuery()
                .or()
                .taskId("invalid")
                .processInstanceIdIn(Collections.singletonList("unexisting")).count());
    }
    ...
    public void testNativeQuery() {
        ...
        assertEquals(1,
                taskService.createNativeTaskQuery().sql("SELECT count(*) FROM " + managementService.getTableName(org.flowable.task.service.Task.class) + " T1, " + managementService.getTableName(VariableInstanceEntity.class) + " V1 WHERE V1.TASK_ID_ = T1.ID_").count());
        List<org.flowable.task.service.Task> tasks = taskService.createNativeTaskQuery().sql("SELECT * FROM " + managementService.getTableName(org.flowable.task.service.Task.class) + " T1, " + managementService.getTableName(VariableInstanceEntity.class) + " V1 WHERE V1.TASK_ID_ = T1.ID_").list();
        assertEquals(1, tasks.size());
        ...
        // use parameters
        assertEquals(1, taskService.createNativeTaskQuery().sql("SELECT count(*) FROM " + managementService.getTableName(org.flowable.task.service.Task.class) + " T WHERE T.NAME_ = #{taskName}").parameter("taskName", "gonzoTask").count());
    }
