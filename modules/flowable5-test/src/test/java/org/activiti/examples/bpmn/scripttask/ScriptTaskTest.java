...
public class ScriptTaskTest extends PluggableFlowableTestCase {
    ...
    public void testDynamicScript() {
        ...
        assertEquals(42, (Number) Integer.valueOf(runtimeService.getVariable(processInstance.getId(), "test").toString()));
        taskService.complete(taskService.createTaskQuery().singleResult().getId());
        ...
        assertEquals(22, (Number) Integer.valueOf(runtimeService.getVariable(processInstance.getId(), "test2").toString()));
        taskService.complete(taskService.createTaskQuery().singleResult().getId());
        assertProcessEnded(processInstance.getId());
    }
