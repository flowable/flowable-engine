    ...
    protected void executeValidation(BpmnModel bpmnModel, Process process, ProcessValidationContext validationContext) {
        ...
        for (SendTask sendTask : sendTasks) {
            ...
            if (StringUtils.isEmpty(sendTask.getType()) && !ImplementationType.IMPLEMENTATION_TYPE_WEBSERVICE.equalsIgnoreCase(sendTask.getImplementationType())) {
                validationContext.addError(Problems.SEND_TASK_INVALID_IMPLEMENTATION, process, sendTask, "One of the attributes 'type' or 'operation' is mandatory on sendTask");
            }
            ...
            if (StringUtils.isNotEmpty(sendTask.getType())) {
                ...
                if (!"mail".equalsIgnoreCase(sendTask.getType()) && !"camel".equalsIgnoreCase(sendTask.getType())) {
                    validationContext.addError(Problems.SEND_TASK_INVALID_TYPE, process, sendTask, "Invalid or unsupported type for send task");
                }

                if ("mail".equalsIgnoreCase(sendTask.getType())) {
                    validateFieldDeclarationsForEmail(process, sendTask, sendTask.getFieldExtensions(), validationContext);
                }

            }

            // Web service
            verifyWebservice(bpmnModel, process, sendTask, validationContext);
        }
    }

    protected void verifyWebservice(BpmnModel bpmnModel, Process process, SendTask sendTask, ProcessValidationContext validationContext) {
        if (ImplementationType.IMPLEMENTATION_TYPE_WEBSERVICE.equalsIgnoreCase(sendTask.getImplementationType()) && StringUtils.isNotEmpty(sendTask.getOperationRef())) {
            ...
            if (!operationFound) {
                validationContext.addError(Problems.SEND_TASK_WEBSERVICE_INVALID_OPERATION_REF, process, sendTask, "Invalid operation reference for send task");
            }
