 ...
 */
package org.flowable.appengine.test.persistence;
...
public class EntitiesTest {
    ...
    protected Class getAndAssertEntityInterfaceClass(String mappedResource) {
        try {
            Class c = Class.forName("org.flowable.appengine.engine.impl.persistence.entity." + mappedResource + "Entity");
            assertNotNull(c);
            return c;
        } catch (Exception e) {
    ...
    protected Class getAndAssertEntityImplClass(String mappedResource) {
        try {
            Class c = Class.forName("org.flowable.appengine.engine.impl.persistence.entity." + mappedResource + "EntityImpl");
            assertNotNull(c);
            return c;
        } catch (Exception e) {
            fail("Entity interface class for " + mappedResource + " not found");
    ...
    private Set<String> getMappedResources() {
        try {
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                if (resource.startsWith("org/flowable/appengine") && !resource.contains("common.xml")) {
                    resource = resource.replaceAll("org/flowable/appengine/db/mapping/entity/", "");
