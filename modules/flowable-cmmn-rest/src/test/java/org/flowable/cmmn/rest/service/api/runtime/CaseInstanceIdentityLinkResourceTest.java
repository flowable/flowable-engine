    ...
    public void testGetIdentityLinks() throws Exception {
        ...
        for (int i = 0; i < responseNode.size(); i++) {
            ...
            if (!link.get("user").isNull()) {
                if (link.get("user").textValue().equals("johnDoe")) {
                    assertEquals("participant", link.get("type").textValue());
                    assertTrue(link.get("group").isNull());
                    assertTrue(link.get("url").textValue().endsWith(CmmnRestUrls.createRelativeResourceUrl(CmmnRestUrls.URL_CASE_INSTANCE_IDENTITYLINK, caseInstance.getId(), "johnDoe", "participant")));
                    johnDoeFound = true;
                } else if (link.get("user").textValue().equals("john")) {
