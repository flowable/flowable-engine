...
public class TaskIdentityLinkResourceTest extends BaseSpringRestTestCase {
    ...
    public void testGetIdentityLinks() throws Exception {
        ...
        assertEquals(2, taskService.getIdentityLinksForTask(task.getId()).size());
        ...
        for (int i = 0; i < responseNode.size(); i++) {
            ...
            if (!link.get("user").isNull()) {
                if (link.get("user").textValue().equals("john")) {
                    ...
                    assertEquals("johnDoe", link.get("user").textValue());
                    assertEquals("assignee", link.get("type").textValue());
                    assertTrue(link.get("group").isNull());
                    assertTrue(link.get("url").textValue().endsWith(CmmnRestUrls.createRelativeResourceUrl(CmmnRestUrls.URL_TASK_IDENTITYLINK, task.getId(), "users", "johnDoe", "assignee")));
