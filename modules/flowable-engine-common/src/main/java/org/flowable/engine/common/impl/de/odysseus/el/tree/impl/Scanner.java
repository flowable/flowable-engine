...
public class Scanner {
	...
	public enum Symbol {
		...
		DOT("'.'"), LBRACK("'['"), RBRACK("']'"),
		COMMA("','"),
		START_EVAL_DEFERRED("'#{'"), START_EVAL_DYNAMIC("'${'"), END_EVAL("'}'"), REGEX("'r'"),
	...
	static {
		...
		addFixToken(new Token(Symbol.REGEX, "r"));
		...
		addKeyToken(new Token(Symbol.REGEX, "r"));
	...
	protected Token nextEval() throws ScanException {
		...
		switch (c1) {
			...
			case 'r': return fixed(Symbol.REGEX);
			case '.':
				if (!isDigit(c2)) {
