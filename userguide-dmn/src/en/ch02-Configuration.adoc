...
The Flowable DMN engine is configured through an XML file called +flowable.dmn.dcfg.xml+. Note that this is *not* applicable if you're using <<springintegration,the Spring style of building a DMN engine>>.
...
----
DmnEngine processEngine = ProcessEngines.getDefaultProcessEngine()
----
 ...
 *
 * @throws FlowableException if an exception occurs when dispatching the event or when the {@link FlowableEventDispatcher}
 * is disabled.
