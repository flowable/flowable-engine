...
Flowable DMN includes a REST API to the Flowable DMN Engine that can be installed by deploying the flowable-app-rest.war (which includes all Flowable REST APIs) file to a servlet container like Apache Tomcat. However, it can also be used in another web-application by including the servlet and it's mapping in your application and add all flowable-dmn-rest module dependencies to the classpath.
...
The Flowable REST web application (flowable-app-rest) is using Spring Java Configuration for starting the Flowable engines, defining the basic authentication security using Spring security and to define the variable converters for specific variable handling.
...
An example configuration is already in comments in this file. This is also the place to override the default RestResponseFactories. For the DMN engine this can be done by defining a new Spring bean with the name dmnResponsefactory and use your custom implementation class for it.
...
Parameters that are part of the url (e.g. the deploymentId parameter in ++http://host/flowable-rest/dmn-api/dmn-repository/deployments/{deploymentId}++)
...
Parameters added as query-string in the URL (e.g. the name parameter used in ++http://host/flowable-rest/dmn-api/dmn-repository/deployments/?name=Deployment++) can have the following types and are mentioned in the corresponding REST-API documentation:
...
Paging and order parameters can be added as query-string in the URL (e.g. the name parameter used in ++http://host/flowable-rest/dmn-api/dmn-repository/deployments?sort=name++).
...
|resourceId|Yes|String|The id of the resource to get the data for. *Make sure you URL-encode the resourceId in case it contains forward slashes. Eg: use 'decisions%2Fmy-decision.dmn' instead of 'decisions/my-decision.dmn'.*
...
|===============
|Parameter|Required|Value|Description
|decisionTableId|Yes|String|The id of the decision table to get.
...
|===============
|Parameter|Required|Value|Description
|decisionTableId|Yes|String|The id of the decision table to get the resource data for.
...
|===============
|Parameter|Required|Value|Description
|decisionTableId|Yes|String|The id of the decision table to get the model for.
