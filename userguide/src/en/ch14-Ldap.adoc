...
Integrating the LDAP system with Flowable is done by injecting an instance of +org.activiti.ldap.LDAPConfigurator+ in the +configurators+ section of the process engine configuration. This class is highly extensible: methods can be easily overridden and many dependent beans are pluggable if the default implementation would not fit the use case.
