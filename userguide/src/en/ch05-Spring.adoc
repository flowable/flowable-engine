...
Note that for this to work, you need to define a _org.flowable.engine.test.ActivitiRule_ bean in the Spring configuration (which is injected by auto-wiring in the example above).

[source,xml,linenums]
----
<bean id="activitiRule" class="org.flowable.engine.test.ActivitiRule">
...
The Spring Boot - Flowable integration is currently experimental. It has been developer together with Spring committers, but it is still early days. We welcome all to try it out and provide feedback. 
