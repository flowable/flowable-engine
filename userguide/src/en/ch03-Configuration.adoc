...
The Flowable process engine is configured through an XML file called +activiti.cfg.xml+. Note that this is *not* applicable if you're using <<springintegration,the Spring style of building a process engine>>.
...
* Add a Flowable configuration file (__activiti.cfg.xml__) to your classpath, pointing to your database (see <<databaseConfiguration,database configuration section>>)
* Execute the main method of the _DbSchemaCreate_ class

However, often only database administrators can execute DDL statements on a database. On a production system, this is also the wisest of choices. The SQL DDL statements can be found on the Flowable downloads page or inside the Flowable distribution folder, in the +database+ subdirectory. The scripts are also in the engine jar (__flowable-engine-x.jar__), in the package __org/activiti/db/create__ (the __drop__ folder contains the drop statements). The SQL files are of the form
...
|TIMER_SCHEDULED|A timer job has been created and is scheduled for being executed at a future point in time.|+org.activiti...ActivitiEntityEvent+
...
|ACTIVITY_MESSAGE_WAITING|An activity has created a message event subscription and is waiting for its receival.|+org.activiti...ActivitiMessageEvent+
|ACTIVITY_MESSAGE_CANCELLED|An activity for which a message event subscription has been created is cancelled and thus receiving the message will not trigger this particular message anymore.|+org.activiti...ActivitiMessageEvent+
...
|PROCESS_CREATED|A process instance has been created. All basic properties have been set, but variables not yet.|+org.activiti...ActivitiEntityEvent+
|PROCESS_STARTED|A process instance has been started. Dispatched when starting a process instance previously created. The event PROCESS_STARTED is dispatched after the associated event ENTITY_INITIALIZED and after the variables have been set.|+org.activiti...ActivitiEntityEvent+
