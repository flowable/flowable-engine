...
Note that Flowable does not ship with a library that allows one to define such a data source. So you have to make sure that the libraries are on your classpath.
...
Flowable supports Mapped Diagnostic Contexts feature of SLF4j. This basic information is passed to the underlying logger along with what is going to be
logged:
...
None of this information is logged by default. The logger can be configured to show them in desired format, extra to the usual logged messages. For example in
