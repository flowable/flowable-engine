...
|PROCESS_CANCELLED|A process has been canceled. Dispatched before the process instance is deleted from runtime. A process instance can for example be canceled by the API call +RuntimeService.deleteProcessInstance+, by an interruping boundary event on a call activity, ...|+org.flowable...FlowableCancelledEvent+
