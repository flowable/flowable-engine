...
Flowable is a drop-in replacement for Activiti (registered trademark by Alfresco). In the all the following sections you'll notice that the package names, configuration files, etc. use _activiti_. The reason is to make the life of people migrating to Flowable as easy as possible (changing the jar dependencies should be enough), but for new people this might be a bit confusing.  We also see Flowable as the natural evolution of our work on Activiti.
